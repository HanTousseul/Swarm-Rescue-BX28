...
class Navigator:
    ...
    def get_stationary_drone_threats(self) -> List[float]:
        ...
        # Configuration
        STILL_THRESHOLD = 8.0   # Max pixels moved to be 'stationary'
        ...
        for data in semantic_data:
            if data.entity_type == "Drone":
                ...
                glob_y = curr_pos[1] + data.distance * math.sin(abs_angle)
                
                # Spatial Hashing: Group readings by 20x20 pixel areas
                grid_key = (int(glob_x // 20), int(glob_y // 20))
                
                if grid_key not in self.tracked_drones:
                    self.tracked_drones[grid_key] = []
                
                self.tracked_drones[grid_key].append((self.counter, (glob_x, glob_y)))
